...
DEBUG_AUDIO_FILE = os.path.join(
    DEBUG_DIR, "debug_audio.wav")  # Save in debug directory
...
def convert_audio(file_path):
    ...
    with sf.SoundFile(file_path) as sf_audio:
        ...
        if sample_rate != 16000:
            audio_data = librosa.resample(audio_data.astype(
                np.float32), orig_sr=sample_rate, target_sr=16000).astype(np.int16)
...
async def whisper_transcription(audio_file=AUDIO_FILE):
    async with websockets.connect(SERVER_URL) as websocket:
        ...
        print(
            f"[INFO] Audio data sent! Time taken: {send_end_time - send_start_time:.3f} seconds")
        ...
        try:
            ...
            print(
                f"[INFO] Total Latency: {latency_end_time - latency_start_time:.3f} seconds")

            return transcription_text, latency_end_time - latency_start_time

        except asyncio.TimeoutError:
            print(
                f"[ERROR] No response from server within {TIMEOUT_SECONDS} seconds.")
            return None, None
